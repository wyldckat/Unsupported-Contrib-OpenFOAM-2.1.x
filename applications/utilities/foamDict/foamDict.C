...
Usage
    ...
    Change the key \a \<key\> entry to \a \<newKey\>. \<newKey\> must be a
    single word.
...
Note
    ...
    way that OpenFOAM handles dictionary files. In particular in combination
    with the inplace operation (\a -i) this is destructive behaviour and needs
    care on the side of the user.
...
int main(int argc, char *argv[])
{
    ...
    // Check that -key is present if required
    switch(op)
    {
